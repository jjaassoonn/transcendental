...
begin
  ...
  intros x y H, squeeze_simp only [ring_hom.eq_int_cast, int.cast_inj] at H, exact H,
end

theorem poly_int_to_poly_real_C_wd' (a : int) : polynomial.C (a:ℝ) = poly_int_to_poly_real (polynomial.C a) := by squeeze_simp [poly_int_to_poly_real]
theorem poly_int_to_poly_real_C_wd : ∀ a : ℤ, poly_int_to_poly_real_wd (polynomial.C a) := λ _ _, by squeeze_simp [poly_int_to_poly_real]
...
begin
  ext m,
  simp only [poly_int_to_poly_real, polynomial.map_C, ring_hom.eq_int_cast, polynomial.map_X, polynomial.map_pow, rat.cast_coe_int,
 polynomial.map_mul, real.of_rat_eq_cast],
end
...
begin
  ...
  simp only [polynomial.aeval_def, polynomial.eval_X, polynomial.eval₂_C, polynomial.eval_C, ring_hom.eq_int_cast,
 polynomial.eval_pow, polynomial.eval₂_mul, polynomial.eval_mul, rat.cast_coe_int, real.of_rat_eq_cast],
...
begin
    ...
    exact H, linarith, linarith, simp only [add_left_inj] at H, exact H,
end
...
begin
  ...
  },
  {
      ...
      use x.val, squeeze_simp [strange_fun], squeeze_simp [h], squeeze_simp at h,                         -- if x < 0 then x ↦ x
      ...
      simp only [strange_fun], simp only [h, sub_lt_zero, sub_add_cancel, subtype.eta, if_false],                                               -- then x - 1 ↦ x
  },
end
...
begin
 rw [identify, zero_int_n, zero_poly_n], ext, squeeze_simp,
end
...
begin                                                                             -- so the identification process descends to identify
  ...
  have g' := @g p zero_poly_n, squeeze_simp only [identify_0_eq_0] at g',
...
begin                                                                                                                             -- identifunction is also surjective
    ...
    {                                                                                                                             -- i.e. everything in support has non-zero coefficient and
      ...
      rw hto_fun, squeeze_simp only [ne.def, finset.mem_filter, finset.mem_range],
      
      split_ifs,
      exact qm_ne_0,

      rw [not_and] at h, squeeze_simp at hm'_left,
      replace h := h hm'_left, squeeze_simp at h, exfalso, exact qm_ne_0 h,
    ...
    use ⟨p, hp_nat_deg⟩,                                                          -- We claim that this polynomial is identified with q
    {
      ...
      -- m.val ∉ support
      {
        squeeze_simp at h,                                                                -- If m ∉ support, then lhs = rhs = 0
        have h' := h m.2,
...
begin                                                                             -- that non-zero polynomial of degree < n is equipotent to ℤⁿ for n ≥ 1
  let f : (poly_n' n.succ) -> (int_n' n.succ),
  {
    ...
    rw hx at g, exact g, rw [zero_poly_n], squeeze_simp, exact p.2.1,
  },
  ...
  split,
  {
    intros x1 x2 hx, squeeze_simp at hx, rw [identify, function.funext_iff] at hx, squeeze_simp at hx,
...

  {
    ...
    use ⟨p.1, ⟨p_ne_0, p.2⟩⟩,
    rw [subtype.ext], squeeze_simp, assumption,
  },
  done
end
  ...
  begin
    ...
    squeeze_simp at rid, by_cases (r 0 < 0), squeeze_simp [h] at rid, linarith,
    squeeze_simp [h] at rid, linarith,
  end
...
begin                                                                             -- to prove ℤ ≃ ℤ - {0}
  ...
  use f, use g, split,
  {
    intros x1 x2 hx, ext, squeeze_simp [f] at hx, 
    replace hx := function.funext_iff.1 hx, replace hx := hx 0, fin_cases x,
    split_ifs at hx, exact hx, squeeze_simp at h_1,
    ...
    squeeze_simp at h, replace h : 0 = x1 0 ∨ 0 < x1 0, exact eq_or_lt_of_le h,
    cases h, rw <-h at hx, squeeze_simp at hx, linarith,

    have ineq : x1 0 + 1 > 1, exact sub_lt_iff_lt_add.mp h, rw hx at ineq, linarith, squeeze_simp at hx, exact hx,
  },
  {
    intros x1 x2 hx, squeeze_simp [g] at hx, replace hx := subtype.eq hx, exact hx,
  }
end
...
begin
  ...
  use fn, use gn, split,
  {
    intros x1 x2 hx, squeeze_simp [fn] at hx, cases hx with h1 h2, rw function.funext_iff at h1, ext,
    by_cases (x = ⟨n.succ, nat.lt_succ_self n.succ⟩), rw <-h at h2, assumption,
      squeeze_simp [fin.eq_iff_veq] at h, have h2 := x.2, replace h2 : x.1 ≤ n.succ := fin.le_last x,
      have h3 : x.1 < n.succ := lt_of_le_of_ne h2 h,
      have H := h1 ⟨x.1, h3⟩, squeeze_simp at H, exact H,
  },

  {
    intros x1 x2 hx, cases x1 with p1 x1, cases x2 with p2 x2,
    ext; squeeze_simp; squeeze_simp [gn] at hx; rw function.funext_iff at hx, swap,
      generalize hm : (⟨n.succ, nat.lt_succ_self n.succ⟩ : fin n.succ.succ) = m,
      have hm' : m.val = n.succ, rw <-hm, replace hx := hx m, squeeze_simp [hm'] at hx, assumption,
      
      generalize hm : (⟨x.1, nat.lt_trans x.2 (nat.lt_succ_self n.succ)⟩ : fin n.succ.succ) = m,
      replace hx := hx m, have hm' : m.1 ≠ n.succ,
        intro a, rw <-hm at a, squeeze_simp at a, have hx' := x.2, linarith,
      squeeze_simp [hm'] at hx, have hm2 := m.2, replace hm2 : m.1 ≤ n.succ, exact fin.le_last m,
      have hm3 : m.1 < n.succ, exact lt_of_le_of_ne hm2 hm',
      have H : x = ⟨m.1, hm3⟩, rw fin.ext_iff at hm ⊢, squeeze_simp at hm ⊢, exact hm,
      rw H, exact hx,
  },
end
...
begin
  ...
  intro rid, rw function.funext_iff at rid, squeeze_simp at rid,
  have ineq : n.succ < n.succ.succ, exact lt_add_one (nat.succ n),
  replace rid := rid ⟨n.succ, ineq⟩, split_ifs at rid,
  have contra := (strange_fun x.snd).property, squeeze_simp at contra, exact contra rid,

  squeeze_simp at h, exact h,
end
...
begin
    intros x y H, squeeze_simp [gn'] at H, rw function.funext_iff at H, ext, rename x_1 m,
    ...
    have eq1 := strange_fun_inj H, have eq2 : m'.val = m.val, rw <-hm', squeeze_simp,
    have eq3 : x.1 m = x.fst ⟨m'.val, _⟩,
    {
        apply congr_arg, rw fin.eq_iff_veq, squeeze_simp, rw eq2,
    }, rw eq3, rw eq1, apply congr_arg, rw fin.eq_iff_veq, squeeze_simp, rw eq2,
    ...
    squeeze_simp at h, exfalso, exact h,
end
...
begin                                                                             -- we have (ℤ^{n+1} - {0}) ≃ ℤⁿ × ℤ for n ≥ 1
  ...
  use fn', use gn', split,
  {
    intros x1 x2 hx, squeeze_simp [fn'] at hx, cases hx with h1 h2, rw function.funext_iff at h1,
    apply subtype.eq', ext, rename x m,
    by_cases (m.val = n.succ),
      have H1 : m = (⟨n.succ, (nat.lt_succ_self n.succ)⟩ : fin n.succ.succ), 
      rw fin.ext_iff, squeeze_simp, exact h,
      rw H1, assumption,

      have H2 : m.1 < n.succ,  have H2' := fin.le_last m, exact lt_of_le_of_ne H2' h,
      have h1m := h1 ⟨m.1, H2⟩, squeeze_simp at h1m, exact h1m,
  },
  {
    exact gn'_inj,
  }
...
begin
  ...
  by_cases (n = 0), rw h, squeeze_simp,
  rw polynomial.coeff_C, squeeze_simp [h], rw polynomial.coeff_eq_zero_of_nat_degree_lt,
  suffices H : 1 ≤ n, linarith, replace h : n ≠ 0 := h, rw <-nat.lt_one_add_iff, norm_num, exact zero_lt_iff_ne_zero.mpr h,
  rw h, squeeze_simp, intro absurd, conv_rhs at h {rw absurd}, suffices H2 : polynomial.C (0 : int) = 0, conv_rhs at h {rw H2}, exact h1 h,
  ext, squeeze_simp,
end
...
begin
  constructor, swap 3, {
    ...
    intros x, squeeze_simp, ext, rename x_1 m, 
    have hm' : m.1 = 0, have hm1 := m.2, linarith,
    have hm : m = ⟨0, by linarith⟩, rw fin.ext_iff, squeeze_simp, exact hm', rw hm,
  }, {
    intros x, squeeze_simp,
  }
end
