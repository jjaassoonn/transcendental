...
begin
  ...
  {
    intro rid, have contra := polynomial.ext_iff.1 rid, simp at contra, replace contra := contra 0,
    exact coeff_zero_after_change f hf contra,
  },

  {
    intro contra, replace contra := pow_eq_zero contra, replace contra := polynomial.ext_iff.1 contra 1, simp at contra, exact contra,
  }
end
