...
begin
  ...
  {
    replace hk : n ≤ k := nat.lt_succ_iff.mp hk,
    replace hk : n < k ∨ n = k := lt_or_eq_of_le hk,
    cases hk,
    {
      rw [<-deriv_succ, IH hk], simp only [polynomial.derivative_zero],
    },
    {
      rw [<-hk, <-deriv_succ, deriv_X_pow'], simp only [mul_one, nat.sub_self, polynomial.derivative_C, pow_zero], exact le_refl n,
    }
  }
end
...
begin
  ...
  have H := f_p_prod_part_k_ge_1_when_j_lt_p_sub_one p hp n j hj k, simp only [polynomial.C_add, polynomial.C_1, int.coe_nat_succ, finset.mem_range] at H, simp only [finset.mem_range] at hk, replace H := H hk, simp only [finset.mem_range] at hi,
...
begin
  ...
  rw eval_sum', apply finset.sum_eq_zero, intros i hi,
    ...
    have ineq : j - i < p - 1,
    {
      ...
      have ineq : j - i ≤ j := nat.sub_le j i, 
...
begin
  ...
  replace ineq1 : polynomial.eval (j:ℤ)
      ...
      polynomial.eval (j:ℤ)
        (f_bar (∏ (i : ℕ) in finset.range g.nat_degree, (polynomial.X - polynomial.C (↑i + 1)) ^ p)) := le_trans ineq1 ineq2,
