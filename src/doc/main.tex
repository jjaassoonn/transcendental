...
\end{minted}
so that {\tt \small Lean} will treat every occurrence of {\tt \small \_`lhs`\_} as {\tt \small \_rhs\_} verbatim. For example \mintinline{Lean}{notation ℤ`[X]` := polynomial ℤ} will replace the {\tt \small Lean} type {\tt \small polynomial ℤ} with a more family notation of $\mathbb{Z}[X]$.
...
\end{minted}
\end{proof}
% Let us evaluate an upper bound for $J_p(g)$ where $g\in\mathbb{Z}[x]$ such that $\mathrm{eval}_g(e)=0$.
